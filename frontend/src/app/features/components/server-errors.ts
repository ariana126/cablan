...
/**
 * Turns whatever the components gateway threw into the errors `submit()` applies to a create or
 ...
 */
export function mapComponentFormError(
  error: unknown,
  form: FieldTree<ComponentFormModel>,
  ...
  if (problem.type === PROBLEM.entityNotFound) {
    return [rootError('این جز دیگر وجود ندارد. فهرست را تازه‌سازی کنید.')];
  }
...
/** Turns whatever `ComponentsGateway.delete` threw into the message the delete-confirmation dialog shows. */
export function mapDeleteError(error: unknown): string {
  ...
  const problem = toProblemDetails(error);
  if (problem?.type === PROBLEM.entityNotFound) {
    return 'این جز پیش‌تر حذف شده است. فهرست را تازه‌سازی کنید.';
  }

  return 'حذف جز ممکن نشد. دوباره تلاش کنید.';
}
